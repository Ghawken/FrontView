...
{
    class XbmcHttpAudioLibrary : IApiAudioLibrary
    {
        private readonly XbmcHttp _parent;

        public XbmcHttpAudioLibrary(XbmcHttp parent)
        {
            _parent = parent;
        }
        ...
        public Collection<ApiAudioGenre> GetGenres()
        {
            ...
            foreach (var dbline in dblines)
            {
                ...
                var genre = new ApiAudioGenre { IdGenre = XbmcHttp.StringToNumber(dbline[0]), Name = dbline[1], AlbumCount = XbmcHttp.StringToNumber(dbline[2]) };
                genres.Add(genre);
            }
            return genres;
        }
        ...
        public Collection<ApiAudioArtist> GetArtists()
        {
            ...
            foreach (var dbline in dblines)
            {
                ...
                var hash = XbmcHttp.Hash(dbline[1]);
                var hash2 = XbmcHttp.Hash("artist"+dbline[1]);
                var genre = new ApiAudioArtist
                                {
                                    IdArtist = XbmcHttp.StringToNumber(dbline[0]), 
                                    Name = dbline[1], 
        ...
        public Collection<ApiAudioAlbum> GetAlbums()
        {
            ...
            foreach (var dbline in dblines)
            {
                ...
                var album = new ApiAudioAlbum
                                {
                                    IdAlbum = XbmcHttp.StringToNumber(dbline[0]), 
                                    Title = dbline[1],
                                    IdGenre = XbmcHttp.StringToNumber(dbline[2]),
                                    IdArtist = XbmcHttp.StringToNumber(dbline[3]),
                                    Artist = dbline[4],
                                    Genre = dbline[5],
                                    Year = XbmcHttp.StringToNumber(dbline[6]),
                                    Thumb = dbline[7]
                                };
        ...
        public Collection<ApiAudioSong> GetSongs()
        {
            ...
            foreach (var dbline in dblines)
            {
                ...
                {
                    IdSong = XbmcHttp.StringToNumber(dbline[0]),
                    Title = dbline[1],
                    Track = XbmcHttp.StringToNumber(dbline[2]),
                    Duration = XbmcHttp.StringToNumber(dbline[3]),
                    Year = XbmcHttp.StringToNumber(dbline[4]),
                    FileName = dbline[5],
                    IdAlbum = XbmcHttp.StringToNumber(dbline[6]),
                    Album = dbline[7],
                    Path = dbline[8],
                    IdArtist = XbmcHttp.StringToNumber(dbline[9]),
                    Artist = dbline[10],
                    IdGenre = XbmcHttp.StringToNumber(dbline[11]),
                    Genre = dbline[12],
                    Thumb = dbline[13],
                };
