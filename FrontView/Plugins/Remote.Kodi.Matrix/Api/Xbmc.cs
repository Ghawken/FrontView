...
*/
        ...
        public string SortOutPath(string path)
        {
            try
            {
                if (System.String.IsNullOrEmpty(path))
                {
                    Logger.Instance().Trace("SortOUTpath Empty set to null  " , path);
                    return "";
                }
                ...
                if (path.Length >= 4 && TruncatePath(path, 4) == "smb:")
                {
                    ...
                    Logger.Instance().Trace("SortOUTpath Smb changing to UNC ",path);
                    char[] MyChar2 = { 's', 'm', 'b', ':' };
                    path = path.TrimStart(MyChar2);
                    Logger.Instance().Trace("SortOUTPath Smb Begining Trimmed " , path);
                    path = path.Replace(@"/", @"\");
                    Logger.Instance().Trace("SortOUT Escapes replaced Final Result:" , path);

                    if (path.EndsWith(@"\"))
                    {
                        path = path.Remove(path.Length - 1);
                        Logger.Instance().Trace("SortOUT Remove Last Char replaced Final Result:", path);
                    }

                    return path;
                }
                Logger.Instance().Trace("SortOUT Neither End Result :" , path);

                return path;
            }
            catch (Exception ex)
            {
                Logger.Instance().Trace("SortOUT Exception caught " , ex.ToString());
                return "";
            }
        }
        ...
        public string GetDownloadPath(string fileName)
        {
            ...
            if (fileName.StartsWith(@"image://"))
            {
                ...
                Logger.Instance().Trace("GetDownloadPath : ", "Using FileName:::" + fileName);
                var newPath = SortOutPath(HttpUtility.UrlDecode(fileName));
                return newPath;
            }
            else {
                Logger.Instance().Trace("Debug : ", "Using FileName:::" + @"http://" + IP + ":" + Port + "/image/" + HttpUtility.UrlEncode(fileName));
                return @"http://" + IP + ":" + Port + "/image/" + HttpUtility.UrlEncode(fileName);
            }
        }
        ...
        public override bool CheckRemote(string os, string version, string additional, bool force)
        {
            ...
            Logger.Instance().Trace("CheckRemote : ", "os:" + os.ToString() + "  - version:" + version.ToString() + " additional:" + additional.ToString());
            Logger.Instance().Trace("CheckRemote : ", "cOS:" + cOs.ToString() + "  - cVersion:"+cVersion.ToString() + " cAdditional:"+cAdditional.ToString() );
            ...
            Logger.Instance().Trace("CheckRemote : ", "Returned TRUE");
            return true;
        }
        ...
        private void CheckTimerTick(object sender, EventArgs e)
        {
            ...
            //Log("CheckTimerTick:  Calling EventClient");
